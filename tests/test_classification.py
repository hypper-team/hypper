...
def test_CDWC(dfs=DS, weighting_iterations_list = [1,2,5], weighting_normalization_strategy_list=['max','l1','l2']):
    pg = ParameterGrid({
    ...
    for pgi in pg:
        cdwc = CDWC(weighting_iterations=pgi['wi'], weighting_normalization_strategy=pgi['wns'], random_seed=42, verbosity=False)
